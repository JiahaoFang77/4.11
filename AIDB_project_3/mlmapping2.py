...
def insert_image_text_to_db(conn, image_name, text):
    cursor = conn.cursor()
    cursor.execute("INSERT INTO Mlmappings (image_name, text) VALUES (?, ?)", (image_name, text))
    conn.commit()
...
def mlmapping2():
    folder_path = "images_preprocessed"  # Replace with the path to your folder containing the images
